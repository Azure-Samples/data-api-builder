...
const rest_request_base = async (url, httpMethod, httpHeaders) => {
    return fetch(url, {
        method: httpMethod,
        headers: httpHeaders
    }).then(async response => {
...
const get_request_base = async (url, httpHeaders) => {
    return await rest_request_base(url, 'GET', httpHeaders);
}

export const rest_functions = {
    get_articles: async (accessToken) => {
