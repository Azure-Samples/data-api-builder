...
DAB_CONFIG_FILE=""
STORAGE_ACCOUNT=""
IMAGE_NAME="hawaiiacr.azurecr.io/dab:M1.5"
IMAGE_REGISTRY_USER=""
...
EOF
	...
	echo "RESOURCE_GROUP: Resource group where resources will be deployed"
	echo "STORAGE_ACCOUNT: Storage account name that will be created to host the locust file"
	exit 1
fi
...
echo "updating webapp siteConfig" | tee -a log.txt
az webapp update -g $RESOURCE_GROUP -n $APP_NAME --set siteConfig.appCommandLine="dotnet Azure.DataApiBuilder.Service.dll --ConfigFileName /App/config/hawaii-config.AzureSQL.json" \
    -o json >> log.txt
